...
import { Bell, Boxes, CalendarDays, Camera, ChevronLeft, LayoutDashboard, LogOut, Menu, MessageSquare, Package, Search, Settings, ShoppingBag, Users, X } from "lucide-react";
  ...
  return <div className="admin-shell">
    ...
    <aside className={`admin-sidebar ${open ? "is-open" : ""}`}>
      ...
      <nav aria-label="Admin navigatsiya">{navigation.map(item => {
        ...
        const active = item.href === "/admin/products" ? pathname.startsWith("/admin/products") : item.href === "/admin/photo-studio" ? photoStudioRoute : item.href === "/admin/content/home" ? homeContentRoute : item.href === "/admin/leads" && leadsRoute;
    ...
    <div className="admin-main">
      <header className="admin-topbar">
        ...
        <div className="admin-topbar-title"><strong>{photoStudioRoute ? "AI Foto Studio" : leadsRoute ? "Mijoz so‘rovlari" : homeContentRoute ? "Home Page" : "Mahsulotlar"}</strong><span>{photoStudioRoute ? "Mahsulot vizuallarini tayyorlash" : leadsRoute ? "Madina AI orqali kelgan mijoz murojaatlari" : homeContentRoute ? "Bosh sahifa kontentini boshqarish" : "Saytdagi mahsulotlarni boshqarish"}</span></div>
