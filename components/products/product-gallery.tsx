...
export function ProductGallery({ product }: { product: Product }) {
  ...
  return <div className="detail-gallery">
    <div className="detail-main-media" aria-live="polite">
      ...
      <div className="detail-main-media-content" key={slide.key}>{slide.src ? <Image unoptimized={isPublicNeonProductImage(slide.src)} src={slide.src} alt={`${product.name} ${product.model} — ${selected + 1}-rasm`} fill sizes="(max-width: 700px) 100vw, 524px" className="detail-real-image" loading={selected===0?"eager":"lazy"}/> : <Icon size={96} strokeWidth={1.4} aria-label={`${product.name} tasviri o‘rni`}/>}</div>
    </div>
