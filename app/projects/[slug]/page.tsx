...
export default async function ProjectPage({ params }: Props) {
  ...
  return <><Header/><main className="project-detail-page"><div className="container"><nav className="project-breadcrumb"><Link href="/">Bosh sahifa</Link><span>›</span><strong>{project.title}</strong></nav><section className="project-detail-hero"><div><span className="eyebrow">AMALGA OSHIRILGAN LOYIHA</span><h1>{project.title}</h1>{project.shortDescription&&<p>{project.shortDescription}</p>}</div>{project.coverImage&&<Image unoptimized={isPublicNeonImage(project.coverImage)} src={project.coverImage} alt={project.title} width={1200} height={760} sizes="(max-width: 700px) calc(100vw - 40px), 1200px" priority/>}</section>{facts.length>0&&<section className="project-facts">{facts.map(([label,value])=><div key={label}><span>{label}</span><strong>{value}</strong></div>)}</section>}{project.description&&<section className="project-copy"><h2>Loyiha haqida</h2><p>{project.description}</p></section>}{project.images.length>1&&<section className="project-gallery"><h2>Loyiha rasmlari</h2><div>{project.images.slice(1).map((src,index)=><Image unoptimized={isPublicNeonImage(src)} key={src} src={src} alt={`${project.title} — ${index+2}`} width={800} height={560} sizes="(max-width: 700px) calc(100vw - 40px), 600px"/>)}</div></section>}<section className="project-cta"><h2>Shunday loyiha kerakmi?</h2><p>Mutaxassislarimiz talablaringizga mos sovutish yechimini hisoblab beradi.</p><a href="#aloqa">Maslahat olish</a></section></div></main><Footer onProducts/></>;
}
