...
export async function createLead(raw:unknown){
  ...
  let row;
  try{row=await getDb().lead.create({data:{submissionKey:value.idempotencyKey||null,customerName:value.customerName||null,phone:normalizePhone(value.phone)||null,telegram:value.telegram||null,requestType:value.requestType||"Boshqa so‘rov",product:value.product||null,dimensions:value.dimensions||null,capacity:value.capacity||null,temperature:value.temperature||null,region:value.region||null,notes:value.notes||null,source:value.source,chatHistory:value.chatHistory,activities:{create:{type:"LEAD_CREATED"}}}});}
  catch{if(value.idempotencyKey){const existing=await findLeadSubmission(value.idempotencyKey).catch(()=>null);if(existing)return{ok:true as const,id:existing.id,duplicate:true as const};}return{ok:false as const,error:"So‘rovni saqlab bo‘lmadi. Iltimos, qayta urinib ko‘ring."};}
