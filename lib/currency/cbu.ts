...
export async function resolveUsdUzsRate(deps:{fetchOfficial:()=>Promise<unknown>;loadFallback:()=>Promise<UsdUzsRate|null>;persist:(rate:UsdUzsRate)=>Promise<void>;now?:()=>Date}):Promise<UsdUzsRate|null>{
  try{
    const rate=parseOfficialUsdRate(await deps.fetchOfficial(),deps.now?.()??new Date());
    if(!rate)throw new Error("INVALID_CBU_RATE");
    try{await deps.persist(rate);}catch(error){console.error("CBU rate persistence failed",{type:error instanceof Error?error.name:"UnknownError",message:error instanceof Error?error.message:"Unknown persistence error"});}
    return rate;
  }catch(error){
    const fallback=await deps.loadFallback();
...
async function fetchAndPersist(){return resolveUsdUzsRate({fetchOfficial:async()=>{const response=await fetch(CBU_URL,{headers:{Accept:"application/json"},signal:AbortSignal.timeout(8000)});console.info("CBU rate HTTP response",{status:response.status});if(!response.ok)throw new Error(`CBU_HTTP_${response.status}`);const payload:unknown=await response.json();const usd=Array.isArray(payload)?payload.find(item=>!!item&&typeof item==="object"&&(item as CbuRecord).Ccy==="USD") as CbuRecord|undefined:undefined;console.info("CBU USD payload",{usdRecordExists:Boolean(usd),rate:typeof usd?.Rate==="string"?usd.Rate:null,nominal:typeof usd?.Nominal==="string"?usd.Nominal:null,effectiveDate:typeof usd?.Date==="string"?usd.Date:null});return payload;},loadFallback,persist:async rate=>{await getDb().siteSettings.upsert({where:{id:"global"},create:{id:"global",usdToUzs:rate.rate,currencySource:"CBU",cbuNominal:rate.nominal,cbuEffectiveDate:new Date(rate.effectiveDate),cbuLastSyncedAt:new Date(rate.lastSuccessfulSync)},update:{usdToUzs:rate.rate,currencySource:"CBU",cbuNominal:rate.nominal,cbuEffectiveDate:new Date(rate.effectiveDate),cbuLastSyncedAt:new Date(rate.lastSuccessfulSync)}});}});}
const getCachedRate=unstable_cache(fetchAndPersist,["official-cbu-usd-uzs-rate-v2"],{revalidate:3600,tags:["cbu-usd-rate-v2"]});
export async function getUsdUzsRate():Promise<UsdUzsRate|null>{try{return await getCachedRate();}catch(error){console.error("CBU rate unavailable",{type:error instanceof Error?error.name:"UnknownError",message:error instanceof Error?error.message:"Unknown CBU error"});try{return await loadFallback();}catch(fallbackError){console.error("CBU fallback unavailable",{type:fallbackError instanceof Error?fallbackError.name:"UnknownError",message:fallbackError instanceof Error?fallbackError.message:"Unknown fallback error"});return null;}}}
