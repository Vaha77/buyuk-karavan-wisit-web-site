...
const DB_SCHEMA_REVISION = "20260923130000";
...
export function getDb(): PrismaClient {
  ...
  if (existing) {
    const delegates = existing as PrismaClient & { project?: { findMany?: unknown }; productCategory?: { findMany?: unknown }; siteSettings?: { findUnique?: unknown } };
    if (typeof delegates.project?.findMany === "function" && typeof delegates.productCategory?.findMany === "function" && typeof delegates.siteSettings?.findUnique === "function" && globalForDb.buyukKaravanDbRevision === DB_SCHEMA_REVISION) return existing;
